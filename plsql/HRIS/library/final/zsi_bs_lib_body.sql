...
BEGIN
   ...
   END IF;
   htp.p('    <input '|| l_type ||' class="form-control input-sm" id="' || p_inputName || '" name="' || p_inputName || '" ' || l_inputValue || l_placeholder || l_attributes || '>');
...
BEGIN
  ...
  IF p_labelSize IS NOT NULL THEN
   ...
   htp.p(l_label);
   htp.p('  <div class="' || l_inputSize || '">');
   htp.p('    <textarea class="form-control input-sm" id="' || p_inputName || '" name="' || p_inputName || '" '  || l_attributes || '>' || l_inputValue  || '</textarea>');
   htp.p('  </div>');
